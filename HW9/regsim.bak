...
; this helps me get the index
; of the instruction in the label list in
; the machine. 
(define (element-index element lst)
  (cond ((equal? element (car lst)) 0)
        (else (+ (element-index element (cdr lst)) 1))))

; this procedure will be used to remove a breakpoint
; from the breakpoint list in the corresponding
; machine. Homework 2 has a question on removing
; elements in a list. 
(define (remove-element element lst)
  (cond ((equal? lst '())'())
        ((equal? element (car lst)) (remove-element element (cdr lst)))
        (else (cons (car lst)(remove-element element (cdr lst))))))
...
(define (make-new-machine)
  (let ((pc (make-register))
        ...
        (breakpoints '()) ; this is the list of breakpoints.
        ; The list below will hold instruction from where
        ; the program will continue when "proceed" is invoked. 
        (break-inst '()) ; 
        ) 

    ; procedure below will set the breakpoints in breakpoints list.
    ; breakpoint will be in list form (offset . label-name)
    (define (breakpoint-install line label)    
      (set! breakpoints (cons (cons line label)
                              breakpoints)))

    ; the procedure below is used for cancelling
    ; a specific breakpoin
    (define (delete-breakpoint line label)
      (let ((br (cons line label)))
        (set! breakpoints (remove-element br breakpoints))))
    
   
    (define (install-l labs)
      (set! labels labs))
    ...
    (let ((the-ops
      ...
      (define (execute)
        (let ((insts (get-contents pc)))
          (if (null? insts)
              'done
              (begin                
                ; if assoc returns true. then the breakpoint is
                ; activated and it will print the offset number
                ; and label

                (if (assoc (element-index (car insts) labels) breakpoints) 
                    (begin
                      (display "Breakpoint: Offset#: ")
                      (display (car (assoc (element-index
                                       (car insts) labels) breakpoints)))
                      (display "  Label: ")
                      (display (cdr (assoc (element-index
                                      (car insts) labels) breakpoints)))
                      (newline)
                      ; saving current instruction so that when the procedure
                      ; "proceed" is invoked, the program will continue from
                      ; that point                        
                      (saved-instruction insts)
                      (newline)
                      )
                    ; code below will resume the program
                    (execute-continue (car insts)))))))              

       ; will be used for when the procedure
       ; proceed is invoked.
       ; the machine will continue running
       ; after the indicated breakpoint
      (define (saved-instruction instruction)
        (set! break-inst (car instruction)))

      ; will resume program. Will call
      ; execute to continue program. 
      (define (execute-continue inst)
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ; setting breakpoint below
              ((eq? message 'set-breakpoint) set-breakpoint)
              ((eq? message 'allocate-register) allocate-register)
              ((eq? message 'get-register) lookup-register)
              ((eq? message 'install-operations)
               (lambda (ops) (set! the-ops (append the-ops ops))))
              ((eq? message 'make-labels) install-l) ; for intalling labels. 
              ; the procedure set-breakpoint will call 'install-breakpoint.
              ; so that the breakpoints will be installed.
              ((eq? message 'breakpoint-install) breakpoint-install)
              ((eq? message 'cancel) delete-breakpoint)
              ; the fallowing message if for deleting all breakpoints.
              ((eq? message 'cancel-all) (set! breakpoints (empty-list)))
              ((eq? message 'stack) stack)             
              ((eq? message 'proceed)
               ; this part, continues machine with instruction after breakpoint.
               ; fallows logic of pc register, which looks at the next
               ; instruction. break-inst holds the instruction from where
               ; to continue in the machine. 
               (execute-continue break-inst)              
               )               
              ((eq? message 'operations) the-ops)                             
              (else (error "Unknown request -- MACHINE " message))))
      dispatch)))

 ; the procedure below will be used for
 ; setting breakpoints in the machine.
(define (set-breakpoint machine label offset)
  ((machine 'breakpoint-install) offset label)
  (newline))

 ; the procedure below will be used for
 ; canceling breakpoints in the machine. 
(define (cancel-breakpoint machine label offset)
  ((machine 'cancel) offset label)
  (newline))

; This procedure will get rid of all the
; breakpoints in the machine.
(define (cancel-all-breakpoints machine)
  (machine 'cancel-all)
  (newline))

; This procedure will help in installing
; the labels in the machine variables list. 
(define (label-installer machine labels)
  ((machine 'make-labels) (car labels)))

; this procedure will help in
; continuing the program after
; the machine hits a breakpoint. 
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
                    ...
                    ; installing the labels here.
                    ; The labels list will be filled here. 
                    (label-installer machine labels)
                    insts )))
      ...
      (extract-labels
       ...
       (lambda (insts labels)             ;; This is where labels and insts are
         (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
           (if (symbol? next-inst)
               ; the if checks for duplicate labels.
               (if (equal? (assoc next-inst labels) #f) 
               (receive insts                              ;; either like this
                        (cons (make-label-entry next-inst  ;;  (it's a label)
                                                insts)
                              labels))
                ; error thrown if duplicate label found.
               (error "ERROR: duplicate labels"))
...
; somehow print what is inside the label list.
; use the get content procedure above to do that.

;(get-contents 'labels)

; (set-breakpoint gcd-machine 'test-b 2) ; just for testing breakpoint list
...
; all this below works. commented with
; ampersand.
...
(start gcd-machine) ; you start the machine once here! It's good!

(proceed-machine gcd-machine)
(cancel-all-breakpoints gcd-machine)
;(newline) ; for some reason this is skipped. shows 6 4
(get-register-contents gcd-machine 'a) ; 40
(get-register-contents gcd-machine 'b) ; 6

(proceed-machine gcd-machine)
(newline) ; for some reason this is skipped. shows 6 4
(get-register-contents gcd-machine 'a) ; 6
(get-register-contents gcd-machine 'b) ; 4
